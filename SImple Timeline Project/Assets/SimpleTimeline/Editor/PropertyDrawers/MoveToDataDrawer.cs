...
[CustomPropertyDrawer(typeof(MoveToData))]
public class MoveToDataDrawer : BaseCommandDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // Delete button (now aligned to the right edge)
        var deleteButtonRect = new Rect(contentRect.xMax - deleteButtonWidth, contentRect.y, deleteButtonWidth, lineHeight);

        // Time inputs (positioned relative to title, not all the way to delete button)
        ...
        EditorGUI.LabelField(titleRect, "Move To Command", EditorStyles.boldLabel);
        ...
        if (foldoutStates[propertyPath])
        {
            ...
            Rect posRect = new Rect(contentRect.x, timeRect.y + lineHeight + spacing, contentRect.width - 50, lineHeight);
            Rect setButtonRect = new Rect(posRect.xMax + 5, posRect.y, 45, lineHeight);
            ...
            if (GUI.Button(setButtonRect, "Set"))
            {
                ...
                if (agent != null)
                {
                    ...
                    targetPositionProp.vector3Value = agent.transform.position;
                    property.serializedObject.ApplyModifiedProperties();
                }
            }
