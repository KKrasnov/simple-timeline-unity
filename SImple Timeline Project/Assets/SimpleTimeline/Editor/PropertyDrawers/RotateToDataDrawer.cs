...
[CustomPropertyDrawer(typeof(RotateToData))]
public class RotateToDataDrawer : BaseCommandDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        EditorGUI.LabelField(titleRect, "Rotate To Command", EditorStyles.boldLabel);
        ...
        if (foldoutStates[propertyPath])
        {
            ...
            Rect rotRect = new Rect(contentRect.x, timeRect.y + lineHeight + spacing, contentRect.width - 50, lineHeight);
            Rect setButtonRect = new Rect(rotRect.xMax + 5, rotRect.y, 45, lineHeight);
            ...
            EditorGUI.PropertyField(rotRect, property.FindPropertyRelative("targetRotation"));
            ...
            if (GUI.Button(setButtonRect, "Set"))
            {
                ...
                if (agent != null)
                {
                    var targetRotationProp = property.FindPropertyRelative("targetRotation");
                    targetRotationProp.vector3Value = agent.transform.eulerAngles;
                    property.serializedObject.ApplyModifiedProperties();
                }
            }
